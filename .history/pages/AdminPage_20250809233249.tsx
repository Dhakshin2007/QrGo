...
import { Loader2, Search, CheckCircle, XCircle, QrCode, ShieldAlert, BadgeInfo, UserCheck, KeyRound, Eye, Users, LogOut, Briefcase, User, ArrowLeft, ToggleLeft, ToggleRight, CalendarCheck2, CalendarX2, Ticket, Clock, HelpCircle } from 'lucide-react';
import { useToast } from '../contexts/ToastContext';
import { useAuth } from '../contexts/AuthContext';
import { useEvents } from '../App';
import { aiService, AISuggestion } from '../services/aiService.ts';
...
const AdminPage: React.FC = () => {
  ...
  const handleEventStatusToggle = (eventId: string, currentStatus: EventStatus) => {
    const newStatus = currentStatus === EventStatus.Ongoing ? EventStatus.Upcoming : EventStatus.Ongoing;
                      ...
                      <div key={event.id} className="bg-surface rounded-lg shadow-lg p-6 flex flex-col justify-between hover:shadow-primary/30 transition-shadow">
                        <div>
                          <div className="flex justify-between items-start">
                              ...
                              <span className={`px-2 py-1 text-xs font-bold rounded-full whitespace-nowrap ${event.status === EventStatus.Ongoing ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>{event.status}</span>
                          </div>
                        ...
                        <div className="mt-4 space-y-2">
                          ...
                          <div className="flex gap-2">
                            <button onClick={() => handleEventStatusToggle(event.id, event.status)} title="Toggle Status" className="w-full p-2 bg-surface hover:bg-background text-on-surface-secondary rounded-md transition-colors flex items-center justify-center gap-2">
                              {event.status === EventStatus.Ongoing ? <><CalendarX2 size={16}/> Set Upcoming</> : <><CalendarCheck2 size={16}/> Set Ongoing</>}
                            </button>
                          </div>
                        </div>
                      </div>
