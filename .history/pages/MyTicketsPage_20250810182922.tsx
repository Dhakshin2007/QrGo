...
import { Booking, Event, EventStatus } from '../types';
...
const MyTicketsPage: React.FC = () => {
  ...
  useEffect(() => {
        ...
        if (isUserLoggedIn && userEmail && userPin && !areEventsLoading) {
            ...
            try {
                ...
                if (foundBookings.length > 0) {
                    const sortedBookings = foundBookings.sort((a, b) => {
                      ...
                      const isAClosed = eventA.status === EventStatus.Closed;
                      const isBClosed = eventB.status === EventStatus.Closed;

                      // Primary sort: push closed events to the bottom
                      if (isAClosed !== isBClosed) {
                          return isAClosed ? 1 : -1;
                      }
                  
                      // At this point, both events are either closed or not closed.
                      const dateA = new Date(eventA.date).getTime();
                      const dateB = new Date(eventB.date).getTime();
                  
                      // Secondary sort:
                      // If both are NOT closed, sort by nearest date (ascending)
                      if (!isAClosed) { 
                          if (dateA !== dateB) return dateA - dateB;
                      }
                      
                      // If both ARE closed, sort by most recent date (descending)
                      if (isAClosed) {
                          if (dateA !== dateB) return dateB - dateA;
                      }
                      
                      // Tertiary sort (tie-breaker): by booking creation time (descending)
    ...
    fetchUserTickets();
  }, [isUserLoggedIn, userEmail, userPin, areEventsLoading, logoutUser, events]);
