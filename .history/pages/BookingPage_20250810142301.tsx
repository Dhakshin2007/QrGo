...
import { ArrowLeft, Loader2, Copy, Check, ExternalLink, User, Mail, Phone, KeyRound, FileText, ClipboardCheck } from 'lucide-react';
                ...
                <div className="flex flex-wrap gap-8 items-center">
                    <div>
                        ...
                        <a href={event.upiLink} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex items-center gap-2 text-primary hover:underline">
                            Pay with UPI App ( Price Mentioned Above ) <ExternalLink size={16}/>
                        </a>
                    </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
           <fieldset className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="relative">
                    ...
                    <input type="text" name="name" placeholder="Full Name" required onChange={handleInputChange} className="w-full bg-background p-3 pl-10 rounded-md focus:ring-2 focus:ring-primary outline-none" />
                </div>
                <div className="relative">
                    <Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-secondary" size={20} />
                    <input type="email" name="email" placeholder="Email Address" required onChange={handleInputChange} className="w-full bg-background p-3 pl-10 rounded-md focus:ring-2 focus:ring-primary outline-none" />
                </div>
                <div className="relative col-span-1 md:col-span-2">
                    <Phone className="absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-secondary" size={20} />
                    <input type="tel" name="phone" placeholder="Phone Number" required onChange={handleInputChange} className="w-full bg-background p-3 pl-10 rounded-md focus:ring-2 focus:ring-primary outline-none" />
                </div>
           </fieldset>
           ...
           {event.requiresEntryNumber && (
             <fieldset>
                ...
                <div className="relative">
                  ...
                  <input type="text" name="entryNumber" placeholder="Entry Number / College ID" required onChange={handleInputChange} className="w-full bg-background p-3 pl-10 rounded-md focus:ring-2 focus:ring-primary outline-none" />
                </div>
             </fieldset>
           )}
                ...
                <fieldset>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="relative">
                            ...
                            <input type="text" name="transactionId" placeholder="Transaction ID / UTR" required onChange={handleInputChange} className="w-full bg-background p-3 pl-10 rounded-md focus:ring-2 focus:ring-primary outline-none" />
                        </div>
           ...
           <fieldset>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="relative">
                        ...
                        <input type="password" name="pin" placeholder="4-Digit PIN" required onChange={handleInputChange} pattern="\d{4}" title="PIN must be exactly 4 digits" maxLength={4} className="w-full bg-background p-3 pl-10 rounded-md focus:ring-2 focus:ring-primary outline-none" />
                    </div>
                    <div className="relative">
                        <KeyRound className="absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-secondary" size={20} />
                        <input type="password" name="confirmPin" placeholder="Confirm PIN" required onChange={handleInputChange} pattern="\d{4}" title="PIN must be exactly 4 digits" maxLength={4} className="w-full bg-background p-3 pl-10 rounded-md focus:ring-2 focus:ring-primary outline-none" />
                    </div>
                </div>
           </fieldset>
           ...
           <button type="submit" disabled={isLoading || event.status === EventStatus.Upcoming} className="w-full bg-accent text-white font-bold py-4 px-6 rounded-md hover:bg-indigo-600 transition-colors text-lg flex items-center justify-center gap-2 disabled:bg-gray-500 disabled:cursor-not-allowed">
              {isLoading && <Loader2 className="animate-spin" />}
              {event.status === EventStatus.Upcoming ? "Booking Not Available" : "Complete Registration"}
           </button>
        </form>
      </div>
    </div>
