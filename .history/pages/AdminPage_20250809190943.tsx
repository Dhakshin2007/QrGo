...
const AdminPage: React.FC = () => {
  ...
  useEffect(() => {
    if (isAdminLoggedIn && view === 'scanner') {
        startScanner();
    }
    return () => {
        stopScanner();
    };
  }, [isAdminLoggedIn, view, startScanner, stopScanner]);
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex justify-between items-center mb-4">
        ...
        <button 
          onClick={() => { logoutAdmin(); setPassword(''); }} 
          className="flex items-center gap-2 bg-surface hover:bg-red-500/50 text-on-surface font-semibold py-2 px-4 rounded-md transition-colors"
        >
            <LogOut size={18} />
            Logout
        </button>
