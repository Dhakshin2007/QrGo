...
const BookingPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!areEventsLoading && eventId) {
      ...
      if (foundEvent) {
        ...
        } else if (foundEvent.status === EventStatus.BookingStopped) {
          setError('Booking for this event has stopped.');
          showToast('Booking for this event is no longer available.', 'info');
        }
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (event?.status === EventStatus.BookingStopped) {
      setError('Booking for this event has stopped.');
      return;
    }
    ...
    <div className="max-w-4xl mx-auto animate-fade-in">
      ...
      <div className="bg-surface rounded-lg shadow-xl p-8">
        ...
        <p className="text-on-surface-secondary mb-6">
            { (() => {
                switch(event.status) {
                    case EventStatus.Upcoming: return 'Booking for this event will open soon.';
                    case EventStatus.Closed: return 'Booking for this event has closed.';
                    case EventStatus.BookingStopped: return 'Booking for this event has stopped.';
            ...
            <div className="flex flex-col-reverse sm:flex-row-reverse gap-4 pt-4 border-t border-background">
                <button type="submit" disabled={isLoading || event.status !== EventStatus.Ongoing} className="w-full sm:flex-1 bg-accent text-white font-bold py-4 px-6 rounded-md hover:bg-indigo-600 transition-colors text-lg flex items-center justify-center gap-2 disabled:bg-gray-500 disabled:cursor-not-allowed">
                  ...
                  {(() => {
                      ...
                      if (event.status === EventStatus.BookingStopped) return "Booking Stopped";
