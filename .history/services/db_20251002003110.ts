...
// FIX: Import base Db types instead of Insert/Update aliases
import { supabase, DbBooking, DbFreeBooking } from './supabaseClient';
...
const getAllBookings = async (): Promise<Booking[]> => {
  ...
  // FIX: All spread operator errors below should be resolved by fixing the Supabase client types.
...
const addBooking = async (
    ...
    if (!event) {
        throw new Error(`Event with ID ${bookingData.eventId} not found.`);
    }
    ...
    if (isPaidEvent) {
        ...
        if (!paymentProofFile) {
            // This check is also on the frontend, but as a safeguard.
            throw new Error("Payment proof is required for paid events.");
        }
        let paymentProofUrl: string | null = null;
        
        // 1. Check for duplicate transaction ID, but only if one is provided
        ...
        // FIX: The insert call error should be resolved by fixing the Supabase client types.
...
const updateBooking = async (updatedBooking: Booking): Promise<Booking> => {
    ...
    if (id.startsWith('free-')) {
        ...
        // FIX: The update call error should be resolved by fixing the Supabase client types.
