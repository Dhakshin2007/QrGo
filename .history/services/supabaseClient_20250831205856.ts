...
// Fix: Use the Row type for Insert as they are identical.
export type DbBookingInsert = DbBooking;

// This explicit type for updates avoids nested generics which can cause TS errors.
// Fix: Use Partial<RowType> for Update, which correctly includes all fields as optional, including the primary key.
export type DbBookingUpdate = Partial<DbBooking>;
...
// Fix: Use the Row type for Insert as they are identical.
export type DbFreeBookingInsert = DbFreeBooking;

// Fix: Use Partial<RowType> for Update.
export type DbFreeBookingUpdate = Partial<DbFreeBooking>;
...
export interface Database {
  public: {
    Tables: {
      ...
      free_bookings: {
        Row: DbFreeBooking;
        Insert: DbFreeBookingInsert;
        Update: DbFreeBookingUpdate;
      };
    };
    Views: {
      [_ in never]: never;
