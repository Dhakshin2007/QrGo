...
const addBooking = async (
    ...
    if (isPaidBooking) {
        ...
        // 4. Create the new paid booking record
        const newBookingPayload = {
            ...
            // entryNumber is not part of paid bookings table
...
const updateBooking = async (updatedBooking: Booking): Promise<Booking> => {
    ...
    if (id.startsWith('free-')) {
        // Destructure to remove properties not in free_bookings table
        const { transactionId, paymentProof, ...freeUpdatePayload } = updatePayload;
        ...
        // Destructure to remove properties not in bookings table
        const { entryNumber, ...paidUpdatePayload } = updatePayload;
